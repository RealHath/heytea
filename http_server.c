...
void http_handler_testget_msg(struct evhttp_request *req, void *arg)
{
	...
	evbuffer_add_printf(retbuff, "Receive GET request!");
...
void http_handler_testpost_msg(struct evhttp_request *req, void *arg)
{
	...
	evbuffer_add_printf(retbuff, "Receive POST request!");
	evhttp_send_reply(req, HTTP_OK, "Client", retbuff);
	evbuffer_free(retbuff);
}
...
void http_handler_index(struct evhttp_request *req, void *arg)
{
	...
	//读取html返回正文
	FILE *fp = fopen("./resource/index.html", "rb");
	if (!fp)
	{
		evhttp_send_reply(req, HTTP_NOTFOUND, "404 not found", NULL);
		return;
	}
	for (;;)
	{
		...
		if (len <= 0)
			break;
		//将要发送给浏览器的内容填充到outbuf
		evbuffer_add(retbuff, buf, len);
	}
	fclose(fp);

	//evbuffer_add(outbuf, "OK\n", 3);
	//发送响应
	evhttp_send_reply(req, HTTP_OK, "Client", retbuff);
	evbuffer_free(retbuff);
}
...
int main()
{
	...
	evhttp_set_cb(http_server, "/", http_handler_index, NULL);
